...
streamline::streamline(const Eigen::Vector3d &startPnt, double xMax, double tol, const Eigen::Vector3d &Vinf,geometry* geom) : Vinf(Vinf),geom(geom)
{
...
Eigen::Vector3d streamline::rkf(const Eigen::Vector3d &x0,double dt,double &error)
{
    ...
    Eigen::Vector3d vPnt = geom->pntVelocity(x0,Vinf);
